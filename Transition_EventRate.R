...
create_event_rate_plots <- function(event_rates, summary_stats, recording_id, 
  ...
  plots$roi_heatmap <- ggplot(roi_epoch_avg, 
    ...
    labs(
      ...
      subtitle = paste("Top", n_rois_to_plot, "most variable ROIs,", window_size, "epoch window"),
      x = "Epoch Position Relative to Transition",
      y = "ROI",
      fill = "Events/epoch"
    ) +
  ...
  plots$roi_trajectories_high <- ggplot(high_activity_trajectories, 
    ...
    labs(
      ...
      subtitle = paste("Top", n_high_activity, "most active ROIs,", window_size, "epoch window (ribbons = ±SE)"),
      x = "Epoch Position Relative to Transition",
      y = "Mean Event Count per Epoch"
    ) +
    theme_minimal() +
  ...
  plots$roi_trajectories_typical <- ggplot(typical_trajectories, 
    ...
    labs(
      ...
      subtitle = paste("Random sample of", n_typical, "typical ROIs,", window_size, "epoch window (ribbons = ±SE)"),
      x = "Epoch Position Relative to Transition",
      y = "Mean Event Count per Epoch"
    ) +
    theme_minimal() +
...
analyze_transition_event_rates <- function(recording_id = RECORDING_ID,
  ...
  cat("==================================================\n")
  cat("Transition Event Rate Analysis\n")
  cat("==================================================\n")
