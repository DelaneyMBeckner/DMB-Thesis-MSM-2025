...
run_animal_pipeline <- function(animal_id,
  ...
  for (condition in conditions) {
    ...
    for (trans_type in transition_types) {
      ...
      if (run_whole_window) {
        cat("     [WHOLE-WINDOW] Computing full window correlations...\n")
        
        # Step 2: Compute CCF
        cat("     [2a/3] Computing cross-correlations...\n")
        ...
        cat("     [2b/3] Analyzing distance-correlation...\n")
      ...
      if (run_trajectory_analysis) {
        cat("     [TRAJECTORY] Running epoch-level trajectory analysis...\n")
        
        # Run CCF trajectory
        cat("     [3a/3] Computing trajectory CCF...\n")
        ...
        # Run distance-correlation trajectory
        cat("     [3b/3] Computing trajectory distance-correlation...\n")
        ...
        # Store trajectory events (with epoch_in_window for event rate trajectories)
        all_trajectory_events[[key]] <- extraction$events %>%
          mutate(
            animal = animal_id,
            condition = condition
          )
  ...
  if (run_whole_window) {
    ...
    cat("Computing percentile rankings for whole-window results...\n")
    ...
    cat("Whole-window percentile rankings complete\n")
  }
  ...
  if (run_trajectory_analysis) {
    cat("Combining trajectory results...\n")
    ...
    cat("  Trajectory events:", nrow(trajectory_events_combined), "observations\n")
  }
  ...
  if (save_intermediate) {
    ...
    if (run_trajectory_analysis) {
      write.csv(trajectory_data_combined,
                file.path(output_dir, paste0(animal_id, "_trajectory_data.csv")),
                row.names = FALSE)
      write.csv(trajectory_dist_combined,
                file.path(output_dir, paste0(animal_id, "_trajectory_dist.csv")),
                row.names = FALSE)
    }
  ...
  return(list(
    ...
    metadata = list(
      ...
      window_size = window_size,
      n_rois = if(!is.null(event_rates_ranked)) length(unique(event_rates_ranked$ROI)) else NA,
      n_pairs = if(!is.null(correlations_ranked)) nrow(correlations_ranked) else NA
    )
  ))
}
