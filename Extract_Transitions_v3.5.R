...
# FIXED: Corrected transition_epoch to use actual epoch values (last epoch of OLD state)
...
load_states <- function(states_file, input_dir = INPUT_DIR) {
  ...
  # Add epoch index if not present (0-indexed to match Time.from.Start)
...
#'   - transition_epoch: The LAST epoch of the OLD state (epoch -1 in window notation)
#'   - transition_time: The START time of the FIRST epoch of the NEW state
#' 
#' Example for Wake->NREM at row 22:
#'   Row 21: epoch=20, state=Wake  <- transition_epoch = 20 (this is epoch -1)
#'   Row 22: epoch=21, state=NREM  <- transition_time = 210 (start of epoch +1)
identify_transitions <- function(states_df) {
  ...
  for (i in 2:nrow(states_df)) {
    ...
    # Check for Wake→NREM (1→2)
    if (prev_state == 1 && curr_state == 2) {
      transitions <- rbind(transitions, data.frame(
        # FIX: Use actual epoch value from the PREVIOUS row (last Wake epoch)
        transition_epoch = states_df$epoch[i - 1],  # Last epoch of OLD state (epoch -1)
        transition_type = "Wake2NREM",
        from_state = "Wake",
        to_state = "NREM",
        transition_time = states_df$Time.from.Start[i]  # Start of FIRST epoch of NEW state
      ))
    }
    
    # Check for NREM→Wake (2→1)
    if (prev_state == 2 && curr_state == 1) {
      transitions <- rbind(transitions, data.frame(
        # FIX: Use actual epoch value from the PREVIOUS row (last NREM epoch)
        transition_epoch = states_df$epoch[i - 1],  # Last epoch of OLD state (epoch -1)
  ...
  cat("  Wake→NREM:", sum(transitions$transition_type == "Wake2NREM"), "\n")
  cat("  NREM→Wake:", sum(transitions$transition_type == "NREM2Wake"), "\n")
  
  return(transitions)
}


#' Extract epoch windows around each transition
#' 
#' Window structure:
#'   - Pre-transition epochs: transition_epoch - window_size + 1  to  transition_epoch
#'     (these become epochs -window_size to -1 in relative notation)
#'   - Post-transition epoch: transition_epoch + 1
#'     (this becomes epoch +1 in relative notation)
#'   - There is NO epoch 0 (T0 is the boundary between epoch -1 and epoch +1)
#' 
#' Example for 9-epoch window with transition_epoch=20:
#'   Pre-transition: epochs 12-20 (relative: -9 to -1)
#'   Post-transition: epoch 21 (relative: +1)
#'   Total: 10 epochs in window
extract_transition_windows <- function(states_df, transitions, window_sizes = c(3, 9)) {
  ...
  for (idx in 1:nrow(transitions)) {
    trans_epoch <- transitions$transition_epoch[idx]  # Last epoch of OLD state
    trans_type <- transitions$transition_type[idx]
    trans_time <- transitions$transition_time[idx]
    
    for (window_size in window_sizes) {
      # Calculate epoch range
      # Pre-transition: window_size epochs ending at trans_epoch
      ...
      # Post-transition: one epoch after trans_epoch
      ...
      # Get time range
      # Time of epoch X is stored in row X+1 (since epochs are 0-indexed, rows are 1-indexed)
      window_start_time <- states_df$Time.from.Start[start_epoch + 1]
      ...
      windows_metadata <- rbind(windows_metadata, data.frame(
        ...
        transition_epoch = trans_epoch,  # Last epoch of OLD state (epoch -1)
        transition_time = trans_time,     # Start time of FIRST epoch of NEW state
        window_size = window_size,
        start_epoch = start_epoch,        # First pre-transition epoch (epoch -window_size)
        end_epoch = end_epoch,            # First post-transition epoch (epoch +1)
...
extract_traces <- function(traces_file, windows_metadata, input_dir = INPUT_DIR) {
  ...
  for (i in 1:nrow(windows_metadata)) {
    ...
    window_traces$transition_id <- trans_id
    
    traces_list[[trans_id]] <- window_traces
  }
  
  cat("Extracted traces for", length(traces_list), "transition windows\n")
...
#' 
#' epoch_in_window calculation:
#'   - Epochs at or before transition_epoch get negative values: -window_size to -1
#'   - Epochs after transition_epoch get positive values: +1, +2, etc.
#'   - There is NO epoch 0
extract_events <- function(events_file, windows_metadata, input_dir = INPUT_DIR) {
  ...
  for (i in 1:nrow(windows_metadata)) {
    ...
    trans_epoch <- windows_metadata$transition_epoch[i]  # Last epoch of OLD state
    ...
    if (nrow(window_events) > 0) {
      # Calculate absolute epoch for each event
      window_events$epoch_abs <- floor((window_events[[time_col]] - start_time) / EPOCH_DURATION) + start_epoch
      
      # Calculate epoch_in_window (relative to transition)
      # trans_epoch is the last epoch of the OLD state (epoch -1)
      # Epochs <= trans_epoch: pre-transition (epochs -window_size to -1)
      # Epochs > trans_epoch: post-transition (epochs +1, +2, etc.)
      window_events$epoch_in_window <- ifelse(
        window_events$epoch_abs <= trans_epoch,
        window_events$epoch_abs - trans_epoch - 1,  # Pre-transition: gives -window_size to -1
        window_events$epoch_abs - trans_epoch       # Post-transition: gives +1, +2, etc.
      )
...
extract_transitions_main <- function(states_file = STATES_FILE,
  ...
  cat("==================================================\n")
  cat("Extract Transitions Analysis (v3 - FIXED)\n")
  cat("==================================================\n")
