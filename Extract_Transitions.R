...
extract_transition_windows <- function(states_df, transitions, window_sizes = c(3, 9)) {
  ...
  for (idx in 1:nrow(transitions)) {
    ...
    for (window_size in window_sizes) {
      ...
      end_epoch <- trans_epoch  # Inclusive
...
extract_events <- function(events_file, windows_metadata, input_dir = INPUT_DIR) {
  ...
  for (i in 1:nrow(windows_metadata)) {
    ...
    if (nrow(window_events) > 0) {
      ...
      window_events$epoch_in_window <- window_events$epoch_abs - windows_metadata$end_epoch[i]  # Relative to transition
      ...
      window_events$Cell_Name <- window_events[[cell_col]]
...
save_outputs <- function(windows_metadata, events_aggregated, recording_id, output_dir) {
  ...
  for (i in 1:nrow(condition_groups)) {
    ...
    meta_file <- file.path(output_dir, paste0(recording_id, "_", trans_type, "_", window_size, "ep_metadata.csv"))
    ...
    if (nrow(condition_events) > 0) {
      events_file <- file.path(output_dir, paste0(recording_id, "_", trans_type, "_", window_size, "ep_events.csv"))
...
extract_transitions_main <- function(states_file = STATES_FILE,
  ...
  cat("==================================================\n")
  cat("Extract Transitions Analysis\n")
  cat("==================================================\n")
  ...
  if (save_output) {
    save_outputs(windows_metadata, events_aggregated, recording_id, output_dir)
  } else {
