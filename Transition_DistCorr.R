...
N_TRAJECTORY_BINS <- 5  # Number of percentile bins for trajectory analysis
...
bin_trajectory_by_distance <- function(trajectory_dist_data, n_bins = 5) {
  ...
  if (n_bins != 5) {
    # Generate generic labels if not 5 bins
...
create_trajectory_distance_plots <- function(binned_results, recording_id, 
  ...
  plots$distance_distribution <- ggplot(distance_data, aes(x = Distance)) +
    ...
    labs(
      ...
      subtitle = paste("Red dashed lines = percentile bin boundaries"),
      x = "Distance (pixels)",
      y = "Count"
    ) +
    theme_minimal() +
  ...
  plots$trajectory_spaghetti <- ggplot(binned_results$bin_epoch_summary) +
    ...
    labs(
      ...
      subtitle = paste(window_size, "epoch window; lines = mean ± SE (shaded ribbons)"),
  ...
  plots$trajectory_faceted <- ggplot() +
    ...
    labs(
      title = paste(recording_id, "-", transition_type, "- Correlation Trajectory by Distance Bin"),
      subtitle = paste(window_size, "epoch window; colored lines = individual transitions; gray = mean"),
      x = "Epoch Position Relative to Transition",
      y = "Mean Pairwise Correlation"
    ) +
    theme_minimal() +
  ...
  plots$bin_characteristics <- ggplot(bin_char_plot,
    ...
    labs(
      ...
      subtitle = "Bars show mean distance, error bars show range",
      x = "Distance Bin",
      y = "Distance (pixels)"
    ) +
    theme_minimal() +
...
analyze_all_transitions <- function(recording_id, transition_type, window_size,
  ...
  for (i in 1:n_transitions) {
    ...
    if (!is.null(linear_fit)) {
      ...
      cat("    Linear fit: slope =", 
          ...
          ", R² =", round(linear_fit$r_squared, 4),
  ...
  cat("  Mean slope:", format(aggregate_stats$mean_slope, scientific = TRUE, digits = 3), 
      "±", format(aggregate_stats$sd_slope, scientific = TRUE, digits = 2), "\n")
  cat("  Mean R²:", round(aggregate_stats$mean_r_squared, 4), "\n")
...
create_distcorr_plots <- function(results, recording_id, transition_type, 
  ...
  if (!is.null(binned_ew) && binning_method %in% c("equal_width", "both")) {
    ...
    plots$binned_averages_equal_width <- ggplot(binned_aggregate_ew, 
      ...
      labs(
        ...
        subtitle = paste(window_size, "epoch window; ribbon = SD, error bars = SE across transitions"),
        x = "Distance (pixels)",
        y = "Mean Correlation"
      ) +
      theme_minimal() +
  ...
  if (!is.null(binned_pct) && binning_method %in% c("percentile", "both")) {
    ...
    plots$binned_averages_percentile <- ggplot(binned_aggregate_pct, 
      ...
      labs(
        ...
        subtitle = paste(window_size, "epoch window; equal-count bins; ribbon = SD, error bars = SE"),
        x = "Distance (pixels)",
        y = "Mean Correlation"
      ) +
      theme_minimal() +
  ...
  if (!is.null(binned_ew) && !is.null(binned_pct) && binning_method == "both") {
    ...
    plots$binning_comparison <- ggplot(combined_binned,
      ...
      labs(
        ...
        subtitle = paste(window_size, "epoch window; ribbon = SD, error bars = SE"),
        x = "Distance (pixels)",
        y = "Mean Correlation",
  ...
  plots$slope_distribution <- ggplot(slopes_df, aes(x = slope)) +
    ...
    labs(
      ...
      subtitle = paste(length(linear_fits), "transitions,", window_size, "epoch window"),
  ...
  # 6. R² distribution
  cat("  6. R² distribution histogram\n")
  ...
  plots$scatter_colored <- ggplot(dist_corr_subset, 
    ...
    labs(
      ...
      subtitle = paste("First", n_to_show, "transitions,", window_size, "epoch window"),
      x = "Distance (pixels)",
      y = "Correlation",
      color = "Transition ID"
    ) +
    ...
    if (!is.null(exp_fit)) {
      ...
      plots$exponential_decay <- ggplot(dist_corr_all, 
        ...
        labs(
          ...
          subtitle = paste("Example from first transition,", window_size, "epoch window"),
          x = "Distance (pixels)",
          y = "Correlation"
        ) +
        theme_minimal() +
...
save_distcorr_results <- function(results, recording_id, transition_type, 
  ...
  filename_all <- file.path(output_dir,
                           paste0(recording_id, "_", transition_type, "_",
                                 window_size, "ep_distcorr_all_pairs.csv"))
  ...
  if (!is.null(results$binned_equal_width)) {
    filename_binned_ew <- file.path(output_dir,
                                paste0(recording_id, "_", transition_type, "_",
                                      window_size, "ep_distcorr_binned_equal_width.csv"))
  ...
  if (!is.null(results$binned_percentile)) {
    filename_binned_pct <- file.path(output_dir,
                                 paste0(recording_id, "_", transition_type, "_",
                                       window_size, "ep_distcorr_binned_percentile.csv"))
  ...
  filename_stats <- file.path(output_dir,
                             paste0(recording_id, "_", transition_type, "_",
                                   window_size, "ep_distcorr_linear_stats.csv"))
  ...
  filename_agg <- file.path(output_dir,
                           paste0(recording_id, "_", transition_type, "_",
                                 window_size, "ep_distcorr_aggregate_stats.csv"))
  ...
  if (!is.null(results$exp_fits)) {
    ...
    filename_exp <- file.path(output_dir,
                             paste0(recording_id, "_", transition_type, "_",
                                   window_size, "ep_distcorr_exp_stats.csv"))
...
save_distcorr_plots <- function(plots, recording_id, transition_type, 
  ...
  for (plot_name in names(plots)) {
    filename <- file.path(output_dir,
                         paste0(recording_id, "_", transition_type, "_",
                               window_size, "ep_distcorr_", plot_name, ".png"))
...
analyze_transition_distcorr <- function(recording_id = RECORDING_ID,
  ...
  cat("\n")
  cat("====================================================\n")
  cat("  TRANSITION DISTANCE-CORRELATION ANALYSIS\n")
  cat("====================================================\n\n")
    ...
    if (save_output) {
      ...
      write.csv(binned_results$binned_data,
               file.path(output_dir, paste0(recording_id, "_", transition_type, "_",
                                           window_size, "ep_distcorr_trajectory_binned.csv")),
               row.names = FALSE)
      
      # Save summaries
      write.csv(binned_results$bin_epoch_summary,
               file.path(output_dir, paste0(recording_id, "_", transition_type, "_",
                                           window_size, "ep_distcorr_trajectory_bin_epoch_summary.csv")),
               row.names = FALSE)
      
      write.csv(binned_results$bin_characteristics,
               file.path(output_dir, paste0(recording_id, "_", transition_type, "_",
                                           window_size, "ep_distcorr_trajectory_bin_characteristics.csv")),
               row.names = FALSE)
      ...
      for (plot_name in names(trajectory_plots)) {
        filename <- file.path(output_dir,
                             paste0(recording_id, "_", transition_type, "_",
                                   window_size, "ep_distcorr_traj_", plot_name, ".png"))
